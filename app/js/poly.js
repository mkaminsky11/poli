...
function plotVerts(v){
	for(var i = 0; i < v.length; i++){
		...
		circle.addEventListener("click", function(){
			...
			if(r === 1){
				this.setAttributeNS(null,'r',3);
				this.setAttributeNS(null,'fill','orange');
			}	
			else{
				this.setAttributeNS(null,'r',1);
...
function plotTri(t,v){
	for(i = t.length; i; ) {
		...
		var pixelData = imCanCtx.getImageData(avgX * scale, avgY * scale, 1, 1).data.slice(0,3);
